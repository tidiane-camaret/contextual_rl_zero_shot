"""
Implicit identification for dynamics adaptation (https://benevans.zip/iida/)
Training loop :
...
def train_agent(args, CARLEnv):
    ...
    predictor = FeedForward(
        d_in=predictor_input_size,
        d_out=int(np.array(envs.single_observation_space.shape).prod()),
        hidden_sizes=[args.hidden_encoder_dim, args.hidden_encoder_dim],
    ).to(device)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                # encode the contexts
                context_mu, context_sigma = context_encoder(data.contexts)
                # detach the context from the graph
                context_mu = context_mu.detach()
                context_sigma = context_sigma.detach()
                ...
                # network taking (s, a, latent) as input and outputting s'
    ...
    eval_context_values = np.linspace(l / 2, u * 2, 40)

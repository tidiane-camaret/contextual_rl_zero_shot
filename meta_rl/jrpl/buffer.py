...
class ReplayBuffer(ReplayBuffer):
    ...
    def sample(self, 
        ...
        if context_id is None:
            ...
            return self._get_samples(batch_inds=sampled_idxs, env=env, add_context=add_context, context_length=context_length)
        else:
            ...
            return self._get_samples(batch_inds=sampled_idxs[0], env_indices=sampled_idxs[1], env=env, add_context=add_context, context_length=context_length)
    ...
    def _get_samples(self, 
        ...
        if add_context:
            ...
            transitions_dim = 2*np.array(self.obs_shape).prod() + np.array(self.action_space.shape).prod()
            ...
            for i in range(len(batch_inds)):
                ...
                # Sample from the context_inds
                ids = np.random.randint(0, context_inds[0].shape[0], size=context_length)
                ...
                contexts[i] = th.tensor(
                                np.concatenate(
                                        (
                                        self._normalize_obs(self.observations[sampled_idxs_from_context[0], sampled_idxs_from_context[1], :], env),
                                        self.actions[sampled_idxs_from_context[0], sampled_idxs_from_context[1], :],
                                        self._normalize_obs(self.next_observations[sampled_idxs_from_context[0], sampled_idxs_from_context[1], :], env),
                                        ),
                                 axis=1)
                                 )

...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self, length=0.5, gravity = 9.8, masscart = 1.0, force_mag = 10.0, oracle = False):
        ...
        high = np.array(
            [
                ...
                np.finfo(np.float32).max,
                np.finfo(np.float32).max # include the pole length in the state (will be 0 if not using oracle)
            ],
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        self.state = (x, x_dot, theta, theta_dot, 0.0 if not self.oracle else self.length)

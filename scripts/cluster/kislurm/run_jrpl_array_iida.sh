...
#SBATCH --array=1-20
# 2 jobs per seed, 10 seeds
seed=$(( ($SLURM_ARRAY_TASK_ID - 1) / 2 ))
script_num=$(( ($SLURM_ARRAY_TASK_ID - 1) % 2 ))
...
case $script_num in
    0)
        ...
        python3 scripts/jrpl/train_${algorithm}.py --track --env-id $env_id --context-name $context_name --seed $seed --context-mode learned_iida \
    ...
    1)
        echo "Running JRPL DQN experiments for $env_id with context $context_name"
        python3 scripts/jrpl/train_ddqn_base_env.py --track --env-id $env_id --context-name $context_name --seed $seed --context-mode hidden_base_env \
            #--learning-rate $learning_rate --batch-size $batch_size --learning-starts $learning_starts --train-frequency $train_frequency \
            #--exploration-fraction $exploration_fraction --start-e $start_e --end-e $end_e --buffer-size $buffer_size

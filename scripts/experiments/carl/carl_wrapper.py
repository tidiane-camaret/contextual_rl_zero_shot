...
def context_wrapper(cls, context_name = "gravity", concat_context = True):
  ...
  class Wrapper(cls):
    ...
    Also gives the option to concatenate the context to the observation
    """
    def __init__(self, concat_context = concat_context, context_name = context_name, *args, **kwargs):
        ...
        if not concat_context:
           ...
           # with shape (self.observation_space.shape[0] + 1,)
            ...
            self.observation_space = Box(
                low=np.concatenate((self.observation_space["obs"].low, self.observation_space["context"][context_name].low)),
                high=np.concatenate((self.observation_space["obs"].high, self.observation_space["context"][context_name].high)),
                dtype=np.float32
            )

    def step(self, action, *args, **kwargs):
        obs, reward, done, truncated, info = super().step(action, *args, **kwargs)
        obs = self._concatenate_obs_context(obs, self.context_name) if self.concat_context else obs["obs"]

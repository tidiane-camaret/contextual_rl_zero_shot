...
#from carl.envs.dmc.carl_dm_walker import CARLDmcWalkerEnv as CARLEnv
...
def make_env(env_id, seed, idx, capture_video, run_name, hide_context):
    def thunk():
        l = CARLEnv.get_default_context()["l"]
        context_distributions = [UniformFloatContextFeature("l", l*0.5,l*2.2)]
...
class QNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.single_observation_space["obs"].shape).prod() + np.prod(env.single_action_space.shape), 256)
...
class Actor(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.single_observation_space["obs"].shape).prod(), 256)
        self.fc2 = nn.Linear(256, 256)
...
"""
    ...
    # env setup
    envs = gym.vector.SyncVectorEnv([make_env(args.env_id, args.seed, 0, args.capture_video, run_name, args.hide_context)])
    ...
    rb = ReplayBuffer(
        args.buffer_size,
        envs.single_observation_space["obs"],
    ...
    while True:
        ...
        action = action.detach().cpu().numpy().clip(envs.single_action_space.low, envs.single_action_space.high)
        action = action[0]
